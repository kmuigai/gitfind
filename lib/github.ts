...
// Layer 2.5: Mid-high repos (10k–100k stars) pushed recently — fills the gap between Layer 2 and Layer 4
export async function searchMidHighRepos(): Promise<GitHubRepo[]> {
  const sevenDaysAgo = new Date()
  sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7)
  const dateStr = sevenDaysAgo.toISOString().split('T')[0]
  ...
  // 2 pages × 100 = 200 recently-pushed repos in the 10k–100k range
  for (let page = 1; page <= 2; page++) {
    const query = encodeURIComponent(`stars:10000..100000 pushed:>${dateStr}`)
    const data = await githubFetch<GitHubSearchResult>(
      `/search/repositories?q=${query}&sort=updated&order=desc&per_page=100&page=${page}`
    )
...
export async function searchHighStarRepos(): Promise<GitHubRepo[]> {
  ...
  // 10 pages × 100 results = 1000 repos, sorted by stars descending
